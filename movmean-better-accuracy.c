...
// rawdata_HB is the recoreded array, and size n is previously defined
  ...
  //scaling things up to reduce error from float-to-int conversion
  for(i=0;i<n;i++)
  {
    rawdata_HB[i] = 32*rawdata_HB[i];
    // rawdata(from analogRead) holds 10-bit values in an int16_t data type, so we can use 6 more bits
    // the multiplier can be increased to up to 63 without overflow
    // please note that this depends on the ADC in your microcontroller, as some have 12-bit ADCs
  }
